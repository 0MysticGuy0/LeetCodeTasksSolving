...
public class FloodFill {
    ...
    private void fillPixel(int[][] image, int sr, int sc, int color, int originalColor){
        if(image[sr][sc] == originalColor && image[sr][sc] != color){
            image[sr][sc] = color;
            if(sr > 0){
                fillPixel(image, sr-1, sc, color, originalColor);
            }if(sr < image.length - 1){
                fillPixel(image, sr+1, sc, color, originalColor);
            }if(sc > 0){
                fillPixel(image, sr, sc-1, color, originalColor);
